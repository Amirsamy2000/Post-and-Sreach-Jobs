...
namespace Employement_Project_MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    Session["User"] = db.Users.Find(User.Identity.GetUserId());
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase PathImage, string code)
        {
            ViewBag.UserType = new SelectList(db.Roles.Where(x=>!x.Name.Contains("admins")).ToList(),"Name","Name");
            ...
            if (ModelState.IsValid) 
            {
                string imageName = Guid.NewGuid() + PathImage.FileName;
                string path = Path.Combine(Server.MapPath("~/Uploads/") , imageName);
                PathImage.SaveAs(path);
                var user = new ApplicationUser {UserName = model.Username, Email = model.Email ,UserType=model.UserType,Country=model.Country,PathImage= imageName,PhoneVisitor=(code+model.PnoneVisitor) };
        ...
        [HttpPost]
        public ActionResult EditProfile(ApplicationUser user, string CurrentPassword, string NewPassword, string code, HttpPostedFileBase imageEdit)
        {
            ViewBag.UserType = new SelectList(db.Roles.Where(x => !x.Name.Contains("admins")).ToList(), "Name", "Name");
            ...
            else
            {
                ...
                if (imageEdit != null)
                {
                    string NewImage = Path.Combine(Server.MapPath("~/Uploads/")+(Guid.NewGuid()+ imageEdit.FileName));
                    string oldImage = Path.Combine(Server.MapPath("~/Uploads/"), user.PathImage);
                    System.IO.File.Delete(oldImage);
                    imageEdit.SaveAs(NewImage);
                }
